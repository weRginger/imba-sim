...
class DARCEST : public TestCache<K, V>
{
public:
...
// Attempt to find existing record
        ...
        if(it_C_1i != C_1i.end()) {
            if(it_C_1i->second.first.getReq().flags & DIRTY) {
                ...
                PRINTV(logfile << "Case I insert key to t2: " << k << "** t1 size: "<< C_1i.size()<< ", t2 size: "<< C_2i.size() <<endl;);

                return (status | PAGEHIT | BLKHIT);
            }
        }
        else if(it_C_2i != C_2i.end()) {
            if(it_C_2i->second.first.getReq().flags & DIRTY) {
        ...
        ///ziqi: DARCER Case III: x hit in C_1o
            ...
            PRINTV(logfile << "ADAPTATION: p increases from " << P_C << " to ";);
            ...
            if(status & DIRTY) {
                ...
                PRINTV(logfile << "Case II insert key to t2: " << k << "** t1 size: "<< C_1i.size()<< ", t2 size: "<< C_2i.size() <<endl;);
        ...
        ///ziqi: DARCER Case IV: x hit in C_2o
            ...
            PRINTV(logfile << "ADAPTATION: p decreases from " << P_C << " to ";);
            ...
            if(status & DIRTY) {
                ...
                PRINTV(logfile << "Case III insert key to t2: " << k << "** t1 size: "<< C_1i.size()<< ", t2 size: "<< C_2i.size() <<endl;);
        ...
        ///ziqi: DARCER Case VII: x is cache miss
        else {
            ///PRINTV(logfile << "Case VII miss on key: " << k << endl;);
            PRINTV(logfile << "Case IV miss on key: " << k << endl;);
            PRINTV(logfile << "Case IV status: ** t1 size: "<< C_1i.size()<< ", t2 size: "<< C_2i.size() <<", b1 size: "<< C_1o.size() <<", b2 size: "<< C_2o.size() <<endl;);
            ///ziqi: Case A
            if((C_1i.size() + C_2i.size() + C_1o.size() + C_2o.size()) == _capacity) {
