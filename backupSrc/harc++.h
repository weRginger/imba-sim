...
#ifndef HARCPlusPlus_H
#define HARCPlusPlus_H
...
class HARCPlusPlus : public TestCache<K, V>
{
public:
...
// the maximum number of records to be stored.
    ...
    uint32_t access(const K &k  , V &value, uint32_t status) {
        ...
        //  Case I: x hit in C_1i or C_2i, then move x to C_2i if it's a read, or to D_2i if it's a write
        if(it_C_1i != C_1i.end()) {
            // if it is a write request
            if(status & WRITE) {
                PRINTV(logfile << "Case I Write hit on C_1i " << k << endl;);

                // HARC++ difference from HARC
                // If a write hit in C_i, how should enlarge desired siez of D_i by one
                if(P > 0) {
                    P--;
                }
        ...
        // Case II: x hit in D_1i or D_2i, then move x to MRU of D_2i
        ...
        // Case III: x hit in C_1o
        else if(it_C_1o != C_1o.end()) {
            // HARC++ difference from HARC
            // If only a read hit in C_1o, we will adjust P and P_C
            if(status & READ) {
