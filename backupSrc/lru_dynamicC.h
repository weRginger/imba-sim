...
#ifndef LRU_DYNAMICC_H
#define LRU_DYNAMICC_H
...
class DynamicCLRUCache : public TestCache<K, V>
{
public:
...
///ziqi: if the seqLength is above the threshold, evict them all
                            if(seqLength > threshold - 1) {
                                ...
                                ///ziqi: if in the cache, we can find consecutive dirty pages longer than threshold, double the threshold.
                                ///ziqi: if not, decrease threshold by 1.
                                threshold*=2;
                                status |= SEQEVICT;
                                evictSth = true;
...
///ziqi: changed at Jun 6 that not only evicting the original dirty page but also other dirty page that sequential to the victim page
                ...
                ///ziqi: if in the cache, no consecutive pages are longer than threshold, decrease it by 1.
                threshold--;
