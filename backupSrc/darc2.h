...
// C++ Interface: hybrid
//
// Description: Dirty/clean cache partition algorithm. Read hit on ghost enlarge desired clean cache; write hit on ghost enlarge desired dirty cache.
//
...
class DARC2 : public TestCache<K, V>
{
public:
...
//         assert ( _capacity!=0 );
    ...
    uint32_t access(const K &k  , V &value, uint32_t status) {
        ...
        ///ziqi: HYBRID Case I: x hit in t1, then move x to t1 if it's a read, or to t2 if it's a write
        ...
        ///ziqi: HYBRID Case II: x hit in t2, then move x to MRU of t2
        else if((it_t2 != t2.end()) && (it_t1 == t1.end())) {
            assert(!((it_t1 != t1.end()) && (it_t2 != t2.end())));
        ...
        ///ziqi: HYBRID Case III: x hit in b1, if it is a read, then enlarge t1, if it is a write, then enlarge t2,
        ///and move x from b1 to t1 if it's a read or to t2 if it's a write
        ...
        ///ziqi: HYBRID Case IV: x hit in b2, if it is a read, then enlarge t1, if it is a write, then enlarge t2,
        ///and move x from b2 to t1 if it's a read or to t2 if it's a write
            ...
            ///ziqi: if it is a write request
            if(status & WRITE) {
                PRINTV(logfile << "Case IV Write Hit on b1: " << k << endl;);
                if(b2.size() >= b1.size()) {
                    delta = 1;
                }
                else {
            ...
            ///ziqi: if it is a read request
                ...
                PRINTV(logfile << "Case IV Read Hit on b1: " << k << endl;);
                if(b1.size() >= b2.size()) {
                    delta = 1;
                }
                else {
        ...
        ///ziqi: HYBRID Case V: x is cache miss
        else {
