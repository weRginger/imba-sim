...
class LDARC : public TestCache<K, V>
{
public:
...
//         assert ( _capacity!=0 );
	...
	static int cr = int(0.1 * 2* _capacity);
...
// Attempt to find existing record
	...
	///ziqi: DARC Case I: x hit in t1, then move x to t1 if it's a read, or to t2 if it's a write
	    ...
	    if(it_t1->second.first.getReq().flags & DIRTY) {
	      ...
	      if( int(0.1 * 2* _capacity) > int(cr - 2*_capacity/(2*_capacity - cr)) )
		cr = (0.1 * 2* _capacity);	    
	      else
		cr = int(cr - 2*_capacity/(2*_capacity - cr));	      
	...
	///ziqi: ARC Case V: x is cache miss  
	    ...
	    if( int(0.9 * 2* _capacity) < int(cr + 2*_capacity/cr) )
	      cr = (0.9 * 2 * _capacity);	    
	    else
	      cr = int(cr + 2*_capacity/- cr);		
	    PRINTV(logfile << cr << endl;);
		...
		qr_key.remove(*itLRU);
	      ...
	      PRINTV(logfile << "Insert key to qr: " << k << ", qr size: "<< qr.size() <<endl;);
	      return (status | PAGEMISS);		    	    
	    }	    
        }
