//
// C++ Interface: FAB with threshold
//		  Evict the largest block that above threshold out of cache when cache is full
//		  If all blocks are below threshold, evict the LRU block
//
// Author: Ziqi Fan, ziqifan16@gmail.com, April 10, 2016
//
//

#ifndef FABPlusPlus_H
#define FABPlusPlus_H
...
// only evict the largest block when above the threshold
extern int threshold;

template <typename K, typename V>
class FABPlusPlus : public TestCache<K, V>
{
public:
    ...
    uint32_t access(const K &k  , V &value, uint32_t status) {
        ...
        if(it == _key_to_value.end()) {
            ...
            if(_key_to_value.size() == _capacity) {
                ...
                // find out the max number of pages in the large block store
                int maxNumber = 0;
                ...
                for(; it_eviction != large_block.end(); it_eviction++) {
                    if(it_eviction->second > maxNumber) {
                        maxNumber = it_eviction->second;
                    }
                }

                /* Different from FAB:
                * Only the maxNumber is above the threshold,
                	 * we will use it as the targe number to select first block near LRU end to evict.
                	 * Otherwise, we will evict the LRU block out.
                	 * */
                int victimLargeBlock = 0;
                if(maxNumber > threshold) {
                    // start from LRU postion to find out the first large block with maxNumber
                    typename key_tracker_type::iterator it_victim = _key_tracker.begin();
                    while(true) {
                        it_eviction = large_block.find(*it_victim);
                        if(it_eviction->second == maxNumber) {
                            victimLargeBlock = it_eviction->first;
                            break;
                        }
                        it_victim++;
                    }
                    ...
                    it_eviction = large_block.find(*(_key_tracker.begin()));
                    victimLargeBlock = it_eviction->first;
                }

                // evict all pages from the victim block
                PRINTV(logfile << "evict block = " << victimLargeBlock << endl;);
                int evcitedPageFromBlock = 0;
                for(int i=0; i<int(_pagePerBlock); i++) {
                    typename key_to_value_type::iterator it_tmp = _key_to_value.find( victimLargeBlock*_pagePerBlock+i);
                ...
                _key_tracker.remove(victimLargeBlock);
                large_block.erase(victimLargeBlock);
