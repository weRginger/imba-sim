...
uint32_t PageMinCache::access(const uint64_t &k  , cacheAtom &value, uint32_t status)
{
    ...
    if(it == _key_to_value.end()) {
        ...
        value.updateFlags(status | (it->second.getReq().flags & DIRTY));
...
// 		if( (++memit)->lineNo != currHeapAtom.lineNo ){ //if it need update do update
        ...
        PRINTV(logfile << "\tHitted key status: " << bitset<10>(value.getReq().flags) << endl;);
...
// 	HeapAtom maxHeapAtom = maxHeap.begin();
    ...
    PRINTV(logfile << "\tBefore eviting, key bit status: " << bitset<10>(it->second.getReq().flags) << endl;);
...
///ziqi: Device_number is set to 1. About Request_flags, 0 is for write and 1 is for read
        ...
        PRINTV(logfile << "\tevicting dirty key " << setit->key <<  endl;);
        totalPageWriteToStorage++;
        PRINTV(logfile << "\tKey dirty bit status: " << bitset<10>(it->second.getReq().flags) << endl;);
        ...
        PRINTV(logfile << "\tevicting clean key without flushing back to DiskSim input trace " << setit->key <<  endl;);

        PRINTV(logfile << "\tKey clean bit status: " << bitset<10>(it->second.getReq().flags) << endl;);
