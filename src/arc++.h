...
// C++ Interface: arc++
//
// Description: when a hit on a page in recency cache, we not only migrate this page to frequency cache,
// 		but also increase the desired size of frequency cache by one
//
//
// Author: Ziqi Fan (UMN)
//
...
#ifndef ARCPlusPlus_H
#define ARCPlusPlus_H
...
class ARCPlusPlus : public TestCache<K, V>
{
public:
    ...
    uint32_t access(const K &k  , V &value, uint32_t status) {
        ...
        if((it_t1 != t1.end()) || (it_t2 != t2.end())) {
            ...
            if((it_t1 != t1.end()) && (it_t2 == t2.end())) {
                ...
                // ARC++ only difference from ARC
                // If a hit in t1, how should enlarge desired siez of t2 by one
                if(p > 0) {
                    p--;
                }
