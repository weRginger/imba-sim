// C++ Interface: FAB: Flash-aware buffer management policy for portable media players
//
// Description:
//
//
...
class FAB : public TestCache<K, V>
{
public:
    ...
    uint32_t access(const K &k  , V &value, uint32_t status) {
        ...
        if(it == _key_to_value.end()) {
            ...
            if(_key_to_value.size() == _capacity) {
                ...
                PRINTV(logfile << "evict lru block = " << maxLargeBlock << endl;);
