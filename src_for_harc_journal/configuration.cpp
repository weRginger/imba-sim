...
uint64_t Configuration::myString2intConverter(std::string str)
{
    ...
    try {
        tempInt = boost::lexical_cast<double>(str) ;
    }
    catch(...) {
...
bool Configuration::read(int argc, char **argv)
{
    ...
    try {
        ...
        if(! tempStr.empty()) {
            priceDRAMvsNVM = (double)boost::lexical_cast<double>(tempStr) ;
        }

        ///ziqi: read out the value for moneyAllocation4DRAM
        try {
            tempStr = pTree.get<std::string>("Global.moneyAllocation4DRAM");
        }
        catch(...) {
            //no log file specified
            tempStr.clear();
        ...
        if(! tempStr.empty()) {
            moneyAllocation4DRAM = (double)boost::lexical_cast<double>(tempStr) ;
        }
        ...
        for(int i = 0; i < totalLevels ; ++i) {
            ...
            ///ziqi: read out afterCacheTrace name from cfg file
            try {
                tempStr = pTree.get<std::string>("Global.afterCacheTrace");
            }
            catch(...) {
                //no log file specified
                tempStr.clear();
