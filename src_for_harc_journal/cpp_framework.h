...
{
public:
    inline_ static tick_t read_cpu_ticks() {
        return RDTICK();
    }
...
{
public:
    inline_ static double ceil(double a) {
        return ::ceil(a);
    }
    inline_ static double floor(double a) {
        return ::floor(a);
    }
};
...
public:
    ...
    inline_ static void *byte_malloc(const size_t size) {
        return malloc(size);
    }
    inline_ static void  byte_free(void *mem) {
        free(mem);
    }
    ...
    inline_ static void *byte_aligned_malloc(const size_t size, const size_t alignment) {
        return ALIGNED_MALLOC(size, alignment);
    }
    inline_ static void  byte_aligned_free(void *mem) {
        ALIGNED_FREE(mem);
    }

    inline_ static void  read_write_barrier() {
        MB();
    }
    inline_ static void  write_barrier() {
        WMB();
    }
    inline_ static void  read_barrier() {
        RMB();
    }
...
public:
    ...
    inline_ bool compareAndSet(const unsigned expect, const unsigned update) {
...
public:
    explicit AtomicReference() {
        _value = null;
    }
    explicit AtomicReference(V *value) {
        _value.set(value);
    }
...
private:
    VolatileType<V *> _value;
public:
    explicit AtomicMarkableReference() {
        _value = 0;
    }
...
public:
    Thread(unsigned int stackSize = 0, bool is_create_only_on_start = false) : _is_start(0), _maxEndCallbacks(0), _is_create_only_on_start(is_create_only_on_start), _stackSize(stackSize) {
        ...
        if(!_is_create_only_on_start) {
            ...
            //-create the thread ------------------------------
            pthread_create(&_handle, &tattr, threadFunc, (void *)this);
...
public://general int functionality
    ...
    //ARH
    inline static bool is_prime(const _u64 n) {
        _u64 i;
    ...
    //ARH
    inline static _u64 nextPrime(const _u32 i) { //Biplob implementation
        _u32 n = i ;
    ...
    inline static _u64 nextDivisible(const _u64 i, const _u32 d) {
        return i + (d - i % d) ;
    }

    inline_ static _u32 log2(const _u64 i) {
        _u32 mask = 1;
        _u32 count = 0;
    ...
    inline_ static std::string IntToStr(int n) {
...
public://NOT THREAD SAFE, EACH THREAD SHOULD USE DIFFRENT OBJECT !!
    ...
    inline_ bool nextBoolean() {
        return (0 != (nextInt(1024) & 1));
    }
